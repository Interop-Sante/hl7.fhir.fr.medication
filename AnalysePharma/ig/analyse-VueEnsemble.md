# L'analyse pharmaceutique - Vue d'ensemble - Guide d'implémentation du médicament v0.1.0
...
La prise en compte du résultat d’une analyse pharmaceutique est codée via l’attribut `Task.businessStatus`. Elle repose sur le jeu de valeurs [FrPharmaceuticalInterventionDevenirCodeValueSet](ValueSet-fr-pharmaceutical-intervention-devenir-code-value-set.md) qui reprend les valeurs du formulaire d’intervention pharmaceutique de la SFPC sans la valeur “Non renseigné” qui est le statut par défaut lorsqu’il n’y a pas d’information sur la prise en compte du résultat d’une analyse pharmaceutique et donc correspond à une absence d’utilisation de l’attribut `Task.businessStatus`.
